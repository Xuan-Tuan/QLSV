...
export default memo( function ParentDetailCoursePage ()
{
	...
	{
		const response = await API_SERVICE.get( `courses/${ code }/attendances`, { parentId: currentUser?.parentId } );
		if ( response?.status == 'success' )
		{
			let schedules = response?.data?.schedules;
			let uniqueDates = [ ...new Set( schedules?.map( schedule => schedule?.dateSche ) ) ];
