				...
				<div className="mb-4">
					...
					>
						...
						<option value={ parentID } selected={parent.parentId == parentID}>
							{ parents.find( ( parent ) => parent.parentId == parentID )?.name || parents.find( ( parent ) => parent.parentId == parentID )?.fullName  }
						</option>
						{ parents?.filter((item) => (!item.hasChild && parent.parentId != parentID))
							.map( ( parent ) => (
								<option key={ parent.id } value={ parent.id } selected={parent.parentId == parentID}>
