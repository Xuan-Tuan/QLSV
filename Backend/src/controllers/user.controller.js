...
const UserController = {
	...
	create: catchAsync( async ( req, res ) =>
	{
		...
		try
		{
			...
			{
				case 'lecturer':
					data = { ...data, lecturerId: request?.lecturerId || generateShortCode() };
				...
				case 'admin':
					...
					break;
				case 'parent':
					data = { ...data, parentId: request?.parentId || generateShortCode() };
					await ParentPersistence.create( data, { transaction } );
					break;
				case 'student':
					data = { ...data, RFID: request?.RFID, parentId: request.parentId, MSSV: request?.MSSV || generateShortCode( 5 ) };
	...
	update: catchAsync( async ( req, res ) =>
	{
		...
		try
		{
			...
			const authenInfo = {
				email: request.email,
				...( request.password && { password: await bcrypt.hash( request.password, 10 ) } ), // Only update password if provided
			};
			...
			switch ( type )
			{
				case 'lecturer':
					data = { ...data, lecturerId: request?.lecturerId || generateShortCode() };
					await LecturerPersistence.update( data, { where: {
						lecturerId: id
					}, transaction } );
					break;
				case 'admin':
					data = { ...data, adminId: request?.adminId || generateShortCode() };
					await AdminPersistence.update( data, { where: {
						adminId: id
					}, transaction } );
					break;
				case 'parent':
					data = { ...data, parentId: request?.parentId || generateShortCode() };
					await ParentPersistence.update( data, { where: {
						parentId: id
					}, transaction } );
					break;
				case 'student':
					data = { ...data, RFID: request?.RFID, parentId: request.parentId, MSSV: request?.MSSV || generateShortCode( 5 ) };
					await StudentPersistence.update( data, { where: {
						MSSV: id
					}, transaction } );
					break;
			}
	...
	getList: catchAsync( async ( req, res ) =>
	{
		try
		{
			...
			if ( list?.length > 0 )
			{
				...
				{
					let newObj = {
						...item.dataValues, authentication: await AuthenticationPersistence.findOne( {
	...
	show: catchAsync( async ( req, res ) =>
	{
		try
		{
			...
			{
				data = {
					...data, authentication: await AuthenticationPersistence.findOne( {
	...
	deleteMethod: catchAsync( async ( req, res ) =>
	{
		...
		try
		{
			...
			{
				case 'lecturer':
					userData = await LecturerPersistence.findOne( { where: { lecturerId: id }, transaction } );
					if ( !userData ) throw new ApiError( 404, `Lecturer with ID ${ id } not found` );
					authId = userData.authId;
					result = await LecturerPersistence.destroy( { where: { lecturerId: id }, transaction } );
					break;
				case 'admin':
					userData = await AdminPersistence.findOne( { where: { adminId: id }, transaction } );
					if ( !userData ) throw new ApiError( 404, `Admin with ID ${ id } not found` );
					authId = userData.authId;
					result = await AdminPersistence.destroy( { where: { adminId: id }, transaction } );
					break;
				case 'parent':
					userData = await ParentPersistence.findOne( { where: { parentId: id }, transaction } );
					if ( !userData ) throw new ApiError( 404, `Parent with ID ${ id } not found` );
					authId = userData.authId;
					result = await ParentPersistence.destroy( { where: { parentId: id }, transaction } );
					break;
				case 'student':
					userData = await StudentPersistence.findOne( { where: { MSSV: id }, transaction } );
					if ( !userData ) throw new ApiError( 404, `Student with ID ${ id } not found` );
					authId = userData.authId;
					result = await StudentPersistence.destroy( { where: { MSSV: id }, transaction } );
					break;
				default:
			...
			await AuthenticationPersistence.destroy( { where: { authId: authId }, transaction } );
