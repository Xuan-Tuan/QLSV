...
const { generateShortCode, generateNumericShortCode, generateSchedule, formattedDate } = require( '../utils/common' );
...
const validateTime = ( timeString ) =>
{
	...
	const date = new Date();
	date.setHours( hours, minutes, 0, 0 ); // Set the time based on hours and minutes
	return date;
}
const controller = {
	...
	create: catchAsync( async ( req, res ) =>
	{
		...
		try
		{
			...
			{
				await Promise.all( request?.students.map( async ( item ) =>
				{
					...
					if ( scheduleList?.length > 0 )
					{
						for ( let e of scheduleList )
						{
							...
							if ( !resultSche )
							{
								...
								await SchedulePersistence.create( resultSche, { transaction } )
							}
							if ( resultSche )
							{
		...
		try
		{
			...
			if ( req?.query?.parentId )
			{
				...
				if ( listStudents?.length > 0 )
				{
					...
					console.log( "listStudentCouse----------> ", studentsIds, listStudentCouse );
					if ( listStudentCouse?.length > 0 )
					{
						for ( let item of listStudentCouse )
						{
			...
			let data = await Promise.all( list.map( async ( item ) =>
			{
				// Fetch students' course data
	...
	deleteMethod: catchAsync( async ( req, res ) =>
	{
		try
		{
			...
			const { id } = req.params;
			let data = await CoursePersistence.findOne( { where: { courseId: id } } );
			if ( !data ) throw new ApiError( 405, `Course with ID ${ id } not found` );
			await CoursePersistence.destroy( { where: { courseId: id } } );
			let result1 = SchedulePersistence.destroy( { where: { courseId: id } } );
			let result2 = AttendancePersistence.destroy( { where: { courseId: id } } );
			...
			if ( course )
			{
				...
				if ( listAtt?.length > 0 )
				{
					for ( let item of listAtt )
					{
						...
						let checkTime = moment( course?.dataValues?.dateSche ).isSameOrAfter( moment( moment().format( 'yyyy-MM-DD' ) ) )
						...
						if ( student )
						{
							listResult.push( {
								...
								attended: item?.dataValues?.attended == "FALSE" ? "Absent" : "Present"
							} )
						}
					}
				}
